...
%row_pivot_normalize(transds=work._weighted_tmout, outtransds=WORK.TMOUTNORM,
      ...
      pivot=0.7,tmt_config=termloc.TextFilter_tmconfig,tmt_train=0,prefix=TextTopic3);

/*initialize topics and termtopics datasets in case they do not exist (0 topics case)*/
%macro tmt_check_topics_exist;
%if(^%sysfunc(exist(termloc.TextTopic3_topics))) %then %do;
   proc sql noprint; create table termloc.TextTopic3_topics
   ...
   quit;
%end;
%if(^%sysfunc(exist(termloc.TextTopic3_termtopics))) %then %do;
   proc sql noprint; create table termloc.TextTopic3_termtopics
   (_topicid decimal, _weight decimal, _termid decimal);
   quit;
%end;
%mend tmt_check_topics_exist;
%tmt_check_topics_exist;
data work.TextTopic3_termtopics; set termloc.TextTopic3_termtopics; run;
data work.TextTopic3_topics; set termloc.TextTopic3_topics; run;
%tmt_doc_score(termtopds=work.TextTopic3_termtopics, docds=&em_score_output,
outds=WORK.TMOUTNORM, topicds=work.TextTopic3_topics, newdocds=work._newdocds, scoring=yes,
termsumds=work._termsumds, prefix=TextTopic3_,pivot=0.7);
